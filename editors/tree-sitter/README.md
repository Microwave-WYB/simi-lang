...
The grammar follows the Rust lexer and parser in `../../src/`, including ASCII identifiers, `--` line comments, string escapes, decimal/exponent numbers, standalone `do ... end` block expressions, destructuring patterns, direct or explicit case/catch arm bodies, and protected `do ... catch of ... end` regions, functional loops, postfix calls/fields/adjacent indexes and `?`, assignment, `|>`/`?>` pipelines, and right-associative trailing arguments. Runtime-category checks use ordinary calls and equality, such as `type(value) == "integer"`; `is` is an ordinary identifier and has no operator or keyword role.
