...
- control flow: `if_expression`, `loop_expression`, `case_expression`, `protected_expression`, `elseif_clause`, and `else_clause`;
- pattern dispatch: `case_expression` contains repeated `case_clause` nodes and `protected_expression` contains repeated `catch_arm` nodes; each arm has a `pattern` field and optional `guard` plus a direct-expression or explicit-block `body` field.

Anonymous nodes must preserve the source spellings used in the query files, including `fn`/`do`/`catch`/`of`/`end`, `?`/`?>`, delimiters, operators, and keywords. Canonical `case` and catch dispatch uses direct single-expression arms or explicit `do ... end` block bodies. Runtime-category checks use ordinary `type(value) == "label"` syntax, while `is` is highlighted as an ordinary identifier. Removed `match ... with`, case arrows, `try`, and `catch pattern do` spellings are outside the query contract; `->` remains the function return-type operator. This is the integration boundary with `editors/tree-sitter`; run `just test-local` after either component changes. Closures are deliberately excluded from function text-object navigation, following Zed's current guidance. Simi has no class-like construct or language injection, so no class text objects or `injections.scm` are provided.
