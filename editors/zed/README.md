...
- pattern dispatch: one `of` on each enclosing expression introduces repeated `case_clause` or `catch_arm` nodes; each arm has a `pattern`, optional `guard`, `=>`, and an expression-valued `body` field.

Anonymous nodes must preserve the source spellings used in the query files, including `fn`/`do`/`catch`/`of`/`end`, `?`/`?>`, delimiters, operators, and keywords. Canonical `case` and catch dispatch uses `pattern [when guard] => expression` arms; `do ... end` is an ordinary arm result expression. Runtime-category checks use ordinary `type(value) == "label"` syntax, while `is` is highlighted as an ordinary identifier. Removed repeated-`of` arms, `match ... with`, thin case arrows, `try`, and `catch pattern do` spellings are outside the query contract; `->` remains the function return-type operator. This is the integration boundary with `editors/tree-sitter`; run `just test-local` after either component changes. Closures are deliberately excluded from function text-object navigation, following Zed's current guidance. Simi has no class-like construct or language injection, so no class text objects or `injections.scm` are provided.
