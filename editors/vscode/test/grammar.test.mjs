...
test("TextMate grammar assigns semantic scopes to representative Simi syntax", async () => {
  ...
  assertScope(lines, 32, "state", "variable.other.readwrite.simi");
  assertScope(lines, 35, "\\q", "invalid.illegal.escape.simi");
  assertScope(lines, 36, "<>", "keyword.operator.concatenation.simi");
  assertScope(lines, 37, "type", "support.function.builtin.simi");
  assertScope(lines, 37, "==", "keyword.operator.comparison.simi");
  assertScope(lines, 40, "|>", "keyword.operator.pipeline.simi");
  assertScope(lines, 37, "\"integer\"", "string.quoted.double.simi");
  assertScope(lines, 38, "type", "support.function.builtin.simi");
  ...
  assertScope(lines, 49, "string", "variable.other.simi");
  assertNotScope(lines, 49, "string", "support.type.primitive.simi");
  assertScope(lines, 49, "to_number", "variable.other.property.simi");
});
