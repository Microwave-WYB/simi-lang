...
test("TextMate grammar assigns semantic scopes to representative Simi syntax", async () => {
  ...
  assertScope(lines, 18, "item", "variable.parameter.simi");
  assertScope(lines, 19, "require", "support.function.builtin.simi");
  assertScope(lines, 20, ".map", "punctuation.accessor.simi");
  assertScope(lines, 20, "map", "variable.other.property.simi", 2);
  assertScope(lines, 20, "<|", "keyword.operator.pipeline.simi");
  assertScope(lines, 23, "raise", "keyword.control.exception.simi");
  assertScope(lines, 24, "do", "keyword.control.block.simi");
  assertScope(lines, 26, "?", "keyword.operator.pipeline.simi");
  assertScope(lines, 27, "state", "variable.other.readwrite.simi");
  assertScope(lines, 30, "\\q", "invalid.illegal.escape.simi");
  assertScope(lines, 31, "type", "support.function.builtin.simi");
  assertScope(lines, 31, "==", "keyword.operator.comparison.simi");
  assertScope(lines, 31, "\"integer\"", "string.quoted.double.simi");
  assertScope(lines, 32, "type", "support.function.builtin.simi");
  assertScope(lines, 32, "==", "keyword.operator.comparison.simi");
  assertScope(lines, 32, "\"function\"", "string.quoted.double.simi");
  assertScope(lines, 33, "is", "variable.other.readwrite.simi");
});
