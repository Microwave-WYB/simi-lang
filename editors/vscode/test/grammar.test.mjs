...
test("TextMate grammar assigns semantic scopes to representative Simi syntax", async () => {
  ...
  assertScope(lines, 25, "map", "variable.other.property.simi", 2);
  assertScope(lines, 25, "<|", "keyword.operator.pipeline.simi");
  assertScope(lines, 28, "raise", "keyword.control.exception.simi");
  assertScope(lines, 29, "do", "keyword.control.block.simi");
  assertScope(lines, 31, "?", "keyword.operator.pipeline.simi");
  assertScope(lines, 33, "state", "variable.other.readwrite.simi");
  assertScope(lines, 38, "\\q", "invalid.illegal.escape.simi");
  assertScope(lines, 39, "<>", "keyword.operator.concatenation.simi");
  assertScope(lines, 40, "type", "support.function.builtin.simi");
  assertScope(lines, 40, "==", "keyword.operator.comparison.simi");
  assertScope(lines, 43, "|>", "keyword.operator.pipeline.simi");
  assertScope(lines, 40, "\"integer\"", "string.quoted.double.simi");
  assertScope(lines, 41, "type", "support.function.builtin.simi");
  assertScope(lines, 41, "==", "keyword.operator.comparison.simi");
  assertScope(lines, 41, "\"function\"", "string.quoted.double.simi");
  assertScope(lines, 42, "is", "variable.other.readwrite.simi");
  assertScope(lines, 44, "identity", "entity.name.function.simi");
  assertScope(lines, 44, "'a", "variable.other.generic-type.simi");
  assertScope(lines, 44, "integer", "support.type.primitive.simi");
  assertScope(lines, 44, "value", "variable.parameter.simi");
  assertScope(lines, 44, "noraise", "storage.modifier.effect.simi");
  assertScope(lines, 45, "raises", "storage.modifier.effect.simi");
  assertScope(lines, 46, "'a", "variable.other.generic-type.simi");
  assertScope(lines, 46, "raises", "storage.modifier.effect.simi");
  assertNotScope(lines, 47, "raises", "storage.modifier.effect.simi");
  assertNotScope(lines, 48, "noraise", "storage.modifier.effect.simi");
  assertScope(lines, 49, "nested", "entity.name.function.simi");
  assertScope(lines, 50, "noraise", "storage.modifier.effect.simi");
  assertScope(lines, 51, "raises", "storage.modifier.effect.simi");
