...
test("TextMate grammar assigns semantic scopes to representative Simi syntax", async () => {
  ...
  assertScope(lines, 36, "state", "variable.other.readwrite.simi");
  assertScope(lines, 41, "\\q", "invalid.illegal.escape.simi");
  assertScope(lines, 42, "<>", "keyword.operator.concatenation.simi");
  assertScope(lines, 43, "type", "support.function.builtin.simi");
  assertScope(lines, 43, "==", "keyword.operator.comparison.simi");
  assertScope(lines, 46, "|>", "keyword.operator.pipeline.simi");
  assertScope(lines, 43, "\"integer\"", "string.quoted.double.simi");
  assertScope(lines, 44, "type", "support.function.builtin.simi");
  assertScope(lines, 44, "==", "keyword.operator.comparison.simi");
  assertScope(lines, 44, "\"function\"", "string.quoted.double.simi");
  assertScope(lines, 45, "is", "variable.other.readwrite.simi");
  assertScope(lines, 47, "identity", "entity.name.function.simi");
  assertScope(lines, 47, "'a", "variable.other.generic-type.simi");
  assertScope(lines, 47, "integer", "support.type.primitive.simi");
  assertScope(lines, 47, "value", "variable.parameter.simi");
  ...
  assertScope(lines, 49, "raises", "storage.modifier.effect.simi");
  assertNotScope(lines, 50, "raises", "storage.modifier.effect.simi");
  assertNotScope(lines, 51, "noraise", "storage.modifier.effect.simi");
  assertScope(lines, 52, "nested", "entity.name.function.simi");
  assertScope(lines, 53, "noraise", "storage.modifier.effect.simi");
  assertScope(lines, 54, "raises", "storage.modifier.effect.simi");
  assertScope(lines, 55, "string", "variable.other.simi");
  assertNotScope(lines, 55, "string", "support.type.primitive.simi");
  assertScope(lines, 55, "to_number", "variable.other.property.simi");
});
