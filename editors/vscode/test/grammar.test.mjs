...
test("TextMate grammar assigns semantic scopes to representative Simi syntax", async () => {
  ...
  assertScope(lines, 46, "noraise", "storage.modifier.effect.simi");
  assertScope(lines, 47, "raises", "storage.modifier.effect.simi");
  assertScope(lines, 48, "'a", "variable.other.generic-type.simi");
  assertScope(lines, 48, "raises", "storage.modifier.effect.simi");
  assertNotScope(lines, 49, "raises", "storage.modifier.effect.simi");
  assertNotScope(lines, 50, "noraise", "storage.modifier.effect.simi");
  assertScope(lines, 51, "nested", "entity.name.function.simi");
  assertScope(lines, 52, "noraise", "storage.modifier.effect.simi");
  assertScope(lines, 53, "raises", "storage.modifier.effect.simi");
