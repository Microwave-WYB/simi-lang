...
test("TextMate grammar assigns semantic scopes to representative Simi syntax", async () => {
  ...
  assertScope(lines, 36, "type", "support.function.builtin.simi");
  assertScope(lines, 36, "==", "keyword.operator.comparison.simi");
  assertScope(lines, 39, "|>", "keyword.operator.pipeline.simi");
  assertScope(lines, 36, "\"integer\"", "string.quoted.double.simi");
  assertScope(lines, 37, "type", "support.function.builtin.simi");
  assertScope(lines, 37, "==", "keyword.operator.comparison.simi");
  assertScope(lines, 37, "\"function\"", "string.quoted.double.simi");
  assertScope(lines, 38, "is", "variable.other.readwrite.simi");
});
