...
fn generate(grammar: &ungrammar::Grammar) -> String {
    ...
    out.push_str("\n#[derive(Clone, Debug, PartialEq, Eq)]\npub enum Stmt { FunctionDecl(FunctionDecl), AliasDecl(AliasDecl), TypeDecl(TypeDecl), LetStmt(LetStmt), ExprStmt(ExprStmt) }\nimpl AstNode for Stmt {\n    fn can_cast(kind: SyntaxKind) -> bool { matches!(kind, SyntaxKind::FUNCTION_DECL | SyntaxKind::ALIAS_DECL | SyntaxKind::TYPE_DECL | SyntaxKind::LET_STMT | SyntaxKind::EXPR_STMT) }\n    fn cast(syntax: SyntaxNode) -> Option<Self> { Some(match syntax.kind() { SyntaxKind::FUNCTION_DECL => Self::FunctionDecl(FunctionDecl::cast(syntax)?), SyntaxKind::ALIAS_DECL => Self::AliasDecl(AliasDecl::cast(syntax)?), SyntaxKind::TYPE_DECL => Self::TypeDecl(TypeDecl::cast(syntax)?), SyntaxKind::LET_STMT => Self::LetStmt(LetStmt::cast(syntax)?), SyntaxKind::EXPR_STMT => Self::ExprStmt(ExprStmt::cast(syntax)?), _ => return None }) }\n    fn syntax(&self) -> &SyntaxNode { match self { Self::FunctionDecl(node) => node.syntax(), Self::AliasDecl(node) => node.syntax(), Self::TypeDecl(node) => node.syntax(), Self::LetStmt(node) => node.syntax(), Self::ExprStmt(node) => node.syntax() } }\n}\n\n");
