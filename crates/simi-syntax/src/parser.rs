...
fn token_name(kind: SyntaxKind, eof: bool) -> &'static str {
    ...
    match kind {
        ...
        SyntaxKind::PANIC_KW => "panic",
        SyntaxKind::TODO_KW => "todo",
...
fn token_lexeme(kind: TokenKind) -> (SyntaxKind, String) {
    let (syntax, text) = match kind {
        ...
        TokenKind::Type => (SyntaxKind::TYPE_KW, "type"),
        TokenKind::Requires => (SyntaxKind::REQUIRES_KW, "requires"),
        ...
        TokenKind::Panic => (SyntaxKind::PANIC_KW, "panic"),
        TokenKind::Todo => (SyntaxKind::TODO_KW, "todo"),
