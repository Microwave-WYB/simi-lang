...
The built-in `list` global provides list-specific mutation, shallow copying, slicing, inspection, and an O(1) snapshot iterator. Generic lazy traversal belongs to `std/iter`; finite sources include `empty`, `once`, `repeat`, and ascending half-open integer `range`, while adapters include `map`, `filter`, `take`, `drop`, `enumerate`, `zip`, and `zip_longest`. Consumers include `to_list`, `fold`, `find`, `find_index`, `contains`, `any`, `all`, `each`, and predicate-based `count`. Iterators are single-pass and sticky after exhaustion. Predicates must return booleans, searches short-circuit and leave later elements unconsumed, callback raises propagate, and `each` returns `nil`. `map.iter` snapshots insertion-ordered `{ key = key, value = value }` entries.
...
Normal/default engines provide the shadowable globals `type(value)`, `inspect(value)`, `require`, `list`, `map`, `iter`, `number`, `string`, and `bytes`. Their canonical portable `std/*` module paths are runtime-owned and require-able without source metadata. The low-level `Interpreter::with_globals` constructor intentionally treats its environment as complete and does not add a prelude. `type` returns the stable reflective labels listed above, including `"function"` for both user and native functions. Detailed runtime diagnostics may still distinguish native functions. `inspect` is cycle-safe human-readable rendering, not serialization.

Modules are registered by the embedding host and cached per `Engine`. Repeated `require` calls return the same cached export value, and module state persists across evaluations performed by that engine. Separate engines have separate module registries. `Engine::new()`, `Engine::with_stdlib()`, root `eval`, and CLI scripts install the runtime-owned portable catalog and its `list`, `map`, `iter`, `number`, `string`, and `bytes` globals. Source `requires` declarations never pin `std`; package manifests retain their top-level `simi` compatibility declaration. The catalog never grants `std/io` or other capability authority. `Engine::builder().build()` remains an explicit bare host configuration.
...
The portable standard library provides shadowable `list`, `map`, `iter`, `number`, `string`, and `bytes` globals plus their require-able canonical `std/*` paths; require-able `std/bytes` is also available as the `bytes` global; `type` and `inspect` are globals. Anonymous functions, trailing callback application, and lazy single-pass iterators are implemented. The CLI additionally registers the opt-in `std/io` text module.
